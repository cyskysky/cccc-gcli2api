...
async def get_max_429_errors_per_credential() -> int:
    """获取单个凭证在被“每日禁用”前的最大连续429错误次数"""
    env_value = os.getenv("MAX_429_ERRORS_PER_CREDENTIAL")
    if env_value:
        try:
            return int(env_value)
        except ValueError:
            pass
    # 默认为 5 次
    return int(await get_config_value("max_429_errors_per_credential", 5))
